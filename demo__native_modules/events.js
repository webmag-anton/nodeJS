...
// добавляем прослушку события someEvent с помощбю метода on('eventName', cb)
emitter.on('someEvent', data => {
	console.log('ON: someEvent', data)  
})

// запускаем (диспатчим) событие someEvent; вторым параметром идут любые данные, которые могут обраблтаться в обработчике
