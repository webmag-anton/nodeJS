...
npm update                       обновляет все пакеты до последней версии
...
npm publish                      публикует пакет в реестре, чтобы его можно было установить по имени {новой версии пакета, измененной 
                                 до публикации в package.json}; комманда выполняется из папки с package.json {корневой дирректории пакета}
...
последующая цифра означает что в проекте произошли глобальные изменения, изменившие как внешнее взаимодействие с проектом, 
так и внутреннюю реализацию. При этом новая версия не совместима с предыдущей.
...
^  - will update you to all future minor/patch versions, without incrementing 
  ...
  Чтобы применить новую версию какого то пакета, нужно обновить ее в package.json, 
^ перед версией не обновит автоматически при переустановке пакетов 
