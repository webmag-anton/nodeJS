...
npm i                            устанавливает все пакеты, указанные в package.json; 
                                 если изменить версии уже установленных пакетов в package.json, то они переустановятся
...
npm i <package>@<version>        устанавливает заданную версию пакета; если уже есть пакет, то переустановится на нужную версию; 
...
npm i --package-lock-only        установка нового лок файла {предварительно нужно удалить старый}
...
create-react-app, который потом создаст проект my-app, а потом create-react-app удалится
  ...
  В папке node_modules у установленных пакетов обычно есть несколько зависимостей (вложенные пакеты), которые можно   
посмотреть в поле dependencies в package.json для каждого пакета. Эти зависимости так же устанавливаются в node_modules. 
...
peerDependencies — говорят, какие зависимости должны быть у родительского проекта, причем у самого 
проекта такая зависимость будет отсутствовать (https://nodejs.org/es/blog/npm/peer-dependencies/).
...
pre-alpha — начальный этап разработки. Фичи сделаны, но еще не протестированы QA специалистом. В них может содержаться 
            большое количество багов.
alpha — внутреннне тестирование. На этом этапе приложение собирается и выкатывается на development стенд, где QA мануальщик 
        проверяет его работу.
beta — публичное тестирование. Условно стабильная сборка приложения, протестированная QA специалистом. Сборка на этом этапы 
       выкладывается на бета стенд для тестирования реальными пользователями.
rc (релиз кандидат) — сборка прошедшая все этапы тестирования и теперь ожидающая своего релиза.
